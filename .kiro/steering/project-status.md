...
# RITM — current project status (auto-loaded)

You are continuing work on **RITM**, a premium local-first SaaS that calculates the real
...
Quick facts:
- Branch: `build-ritm-foundation`. Open PR: #3 → `main`. Push there (never to `main` directly; use the GitHub power, not raw `git push`).
