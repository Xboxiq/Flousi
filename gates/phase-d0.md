...
Surface: `/calculator` as a lit scene + the Coin's debut. Dials: variance 8 · motion 6 · density 6.
Verified 2026-08-17. Proofs eye-checked one by one (6 renders, incl. LTR mirror + loss state).
...
- [x] G2: VISUAL-LAW §2 (overhead light) enforced structurally — ONE side-face brightness token, not two
  ...
  layout mirrors, the coin's highlight stays overhead.

- [x] G3: the Coin is image-grade — ≥3 cooperating light layers + all three shadow roles as named parts
  CHECK: grep -o 'data-part="[a-z-]*"' src/presentation/components/objects/coin.tsx | sort -u
  EXPECT: contact-shadow AND cast-shadow AND occlusion AND specular AND edge
  EVIDENCE: cast-shadow · contact-shadow · edge · engrave · gloss · occlusion ·
  specular · stage — 5 light layers (minted body radial, reeded conic edge masked
  to an annulus, fixed overhead gloss, pointer-tracked specular sweep, engraved
  glyph cut with shadow-above/light-below) + the full shadow trio. Polarity is
  carried by the OBJECT: the coin tarnishes to red on a loss (see
  `light-1440-loss.png`) — state as material, per VISUAL-LAW §8.
...
- [x] G5: scene has three depth planes (field / mid / focal) per VISUAL-LAW §7
  ...
  technical grid, dot-grain) · mid (carved clay work surfaces) · focal (coin
  overlapping the glass result, standing in its own pool of light). The coin's
  shadows land ON the panel — the overlap that proves depth (§1).
...
- [x] G6: motion budget respected — no infinite animation on product components
  ...
  EXPECT: 0
  EVIDENCE: 0 — the coin does not idle-float; it responds to the pointer only. The
  scene animates once on entrance (≤ 700ms) and is then still.

- [x] G7: reduced-motion honored by every new motion surface (coin tilt, scene entrance, ritual)
  CHECK: grep -l "useReducedMotion" coin.tsx ritual-button.tsx calculator-view.tsx | wc -l
  EXPECT: 3
  EVIDENCE: 3/3 — coin skips tilt, scene skips entrance, ritual skips fill+shards
...
- [x] G13: slop scan not regressed (≤ 84 hits) and new suppressions justified
  ...
  EVIDENCE: 84 → 114 → **99** after the material layer was extracted into
